...
test('spojí nákup rajské a šunkofleků, ale nemíchá gramy a lžičky másla', () => {
  ...
  assert.deepEqual(items.filter(item => item.name === 'Máslo').map(item => item.amount).sort(), ['1 lžička', '60 g']);
...
test('převádí kg a litry, násobí oba konce rozmezí a neodhaduje balení ani chybějící množství', () => {
  ...
  assert.equal(list({ [burtgulas.id]: { factor: 2 } }).find(item => item.name === 'Vývar').amount, '5–6 l');
  assert.equal(list({ [recipe('kokosove-kure').id]: { factor: 2 } }).find(item => item.name === 'Rýže').amount, 'neuvedeno');
});
...
test('vlastní množství přežije obnovení a export, ale nepřejde na jinou dávku', () => {
  ...
  const raw = { version: 1, amounts: { [item.key]: { text: '1 sklenice', signature: item.signature } } };
  ...
  const changed = list({ [selected.id]: { factor: 2 } }).find(item => item.name === 'Kyselé okurky');
