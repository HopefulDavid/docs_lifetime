...
import { buildShoppingList, recipeSettings, restoreState } from '../templates/kitchen/public/kitchen-core.mjs';
import { exportShopping, importShopping, previewShoppingImport } from '../templates/kitchen/public/kitchen-transfer.mjs';
...
const selected = (...recipes) => ({ ...restoreState(null, catalog.recipes), selections: Object.fromEntries(recipes.map(recipe => [recipe.id, recipeSettings(recipe)])) });
...
const payloadFor = recipe => ({ version: 1, recipes: [{ id: recipe.id, revision: recipe.revision, factor: 1, choices: {}, enabled: {} }], checked: [], amounts: [] });
...
test('export a odkaz přenesou dávku, volby, hotové a vlastní množství bez filtrů a vaření', async () => {
  ...
  const restored = await importShopping(`https://example.invalid/docs_lifetime/nakup.html#nakup=${code}`, catalog);
...
test('nekomprimovaný formát má stejnou validaci a neplatný vstup nic nepřepisuje', async () => {
  ...
  for (const bad of ['', 'běžný seznam', 'NK1g.xxx', 'NK2g.abc', codeFor(null), codeFor({ ...payload, version: 9 }), codeFor({ ...payload, recipes: [...payload.recipes, ...payload.recipes] })]) {
...
test('odmítne jinou revizi, neznámý recept, chybné volby a cizí suroviny', async () => {
  for (const patch of [{ revision: 'stará' }, { id: 'food/unknown/recipe' }, { factor: 999 }, { choices: { cizi: 0 } }, { enabled: { cizi: true } }, { enabled: null }]) {
    const payload = payloadFor(pasta);
...
test('různé dávky vyžadují výslovnou volbu a staré potvrzení neplatí pro jiné množství', () => {
  ...
  const plan = previewShoppingImport(a, b, catalog, 'merge', { [unresolved.conflicts[0].key]: 'incoming' });
  assert(plan.ready);
