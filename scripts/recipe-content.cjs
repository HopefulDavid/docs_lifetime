...
const validName = value => typeof value === 'string' && identifier(value) && value === value.trim() && !/[|<>\r\n\t\u0000-\u001f]/.test(value);
for (const [category, values] of Object.entries(departments)) {
  ...
  if (normalizedCategories.has(identifier(category))) throw new Error(`data/ingredients.json: duplicitní oddělení '${category}'`);
...
function parseRecipeContent(content, file, { onWarning = () => {} } = {}) {
  let lineNumber = 1;
  const fail = message => { throw new Error(`${file}:${lineNumber}: ${message}`); };
  const ingredients = [];
  const groups = [];
  const steps = [];
  ...
  let table = null;
  const finishTable = () => {
    if (table && (!table.separator || !table.rows)) fail('tabulka surovin musí mít hlavičku, oddělovací řádek a alespoň jednu surovinu');
    table = null;
  };
  ...
  let groupLine = 0;
  const finishGroup = () => {
    if (groupLine && !ingredients.some(item => item.group === group.id)) fail(`skupina surovin '${group.title}' z řádku ${groupLine} nemá žádnou tabulku surovin`);
  };
  ...
  for (const [index, rawLine] of content.replace(/\r\n/g, '\n').split('\n').entries()) {
    ...
    if (/^## /.test(line)) {
      ...
      if (inIngredients) { finishTable(); finishGroup(); }
      inSteps = line === '## Postup';
    }
    ...
    if (/^(?:[-*+] |\d+[.)] )/.test(line)) fail('ingredience musí být v tabulce Surovina / Množství / Upřesnění');
    if (!line.includes('|')) {
      if (line && !line.startsWith('>') && !line.startsWith('<!--') && (table || ingredients.length)) fail('text surovin mimo tabulku; poznámku zapište jako citaci > nebo do Upřesnění');
      continue;
    }
    if (!line.startsWith('|') || !line.endsWith('|')) fail('řádek tabulky surovin musí začínat i končit znakem |');
    ...
    if (cells.join('|') === 'Surovina|Množství|Upřesnění') {
      finishTable();
      table = { separator: false, rows: 0 }; continue;
    }
    if (!table) fail('chybí hlavička tabulky Surovina / Množství / Upřesnění');
    if (!table.separator) {
      if (!cells.every(cell => /^:?-{3,}:?$/.test(cell))) fail('za hlavičkou tabulky musí být oddělovací řádek |---|---|---|');
      table.separator = true; continue;
    }
    ...
    for (const option of options) if (!names.has(option)) {
      const canonical = [...names.keys()].find(name => identifier(name) === identifier(option));
      fail(`neznámá surovina '${option}' v data/ingredients.json; ${canonical ? `použijte přesný název '${canonical}'` : 'doplňte ji právě jednou do odpovídajícího nákupního oddělení'}`);
    }
    ...
    table.rows++;
    if (quantity === 'neuvedeno') onWarning({ severity: 'warning', code: 'RECIPE_QUANTITY_MISSING', file, line: lineNumber,
      ingredient: name, message: `Surovina '${name}' má množství neuvedeno; doplňte ověřené množství ve zdrojovém receptu.` });
  }
  if (inIngredients) { finishTable(); finishGroup(); }
  if (inSteps) finishStep();
