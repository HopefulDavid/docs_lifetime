...
function verifyCatalogAndSearch(search, expectedPages) {
  assert.equal(
...
function verifyPublicLinks(expectedPages) {
...
function verifyStaticRecipes(catalog) {
...
function verifyResources() {
  for (const resource of [
    ...
    assert(
      fs.existsSync(path.join(site, resource)),
      `Chybí veřejný asset nebo licence: ${resource}`,
    );
  }
