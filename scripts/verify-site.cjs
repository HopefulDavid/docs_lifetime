...
const expectedPages = manifest.files.filter(file => file.path.endsWith('.md')).map(file => file.path.replace(/\.md$/, '.html'));
const expectedHtml = manifest.files.filter(file => /\.(md|yml)$/.test(file.path)).map(file => file.path.replace(/\.(md|yml)$/, '.html'));
const actualHtml = fs.readdirSync(site, { recursive: true }).filter(file => file.endsWith('.html')).map(file => file.replace(/\\/g, '/'));

assert.deepEqual(actualHtml.sort(), expectedHtml.sort(), 'Web nesmí obsahovat staré nebo chybějící HTML stránky.');
assert.equal(read('data/recipes.json'), fs.readFileSync(path.join(root, '_generated/data/recipes.json'), 'utf8'), 'Publikovaný katalog musí odpovídat připravenému docsetu.');
assert.deepEqual(Object.keys(search).sort(), [...expectedPages].sort(), 'Fulltext musí zahrnout právě aktuální veřejné stránky.');
...
console.log(`Statický web je ověřený (${expectedPages.length} stránek, ${catalog.recipes.length} receptů, odkazy, fulltext a PDF assety).`);
