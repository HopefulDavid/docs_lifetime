...
| Lokální reprodukce | Spusť úplnou lokální kontrolu z [`../development/commands.md`](../development/commands.md) | `pnpm test` a build projdou, obsahová upozornění nezastaví výsledek | Zastaralý generovaný soubor, vadný odkaz nebo neobnovený nástroj | Oprav nejbližší potvrzenou příčinu |
...
3. Checkoutni stejný zdrojový commit bezpečným projektovým postupem a spusť `pnpm test` a `pnpm run docs:build`.
...
2. Spusť `pnpm run docs:generate` a zkontroluj očekávaný záznam v `_generated/data/recipes.json`, příslušném přehledu a TOC uvnitř `_generated/`.
3. Spusť `pnpm test` a `pnpm run docs:build`.
...
1. Spusť `pnpm test` a potvrď cílené scénáře normalizace i nulového výsledku.
...
1. Spusť `pnpm run test:unit` a potvrď víceletou fixture, počty období, typy commitů, časová pásma a historii oddělenou Git tagem.
2. Spusť `pnpm run docs:generate`, porovnej hlavičku s `git rev-parse HEAD` a `_generated/changelog.md` s `git log` bez ruční úpravy výstupu.
3. Ověř úplný checkout a hodnotu `tag_pattern = "^$"` v kanonickém `cliff.toml`.
4. Spusť `pnpm run docs:build` a potvrď, že `_site/changelog.html` obsahuje historii od nejnovějších záznamů, otevřené nejnovější období, sbalené starší roky, české štítky, odkazy na dostupné články a stabilní kotvy.
...
K 2026-09-21 prošly `pnpm test` a `pnpm run docs:build` v ověřeném prostředí.
...
**Vlastník a uzavření:** Správce prostředí vyřeší dostupnost nástroje podle místní politiky a opravu potvrdí úspěšný `pnpm test` a `pnpm run docs:build` podle [projektových příkazů](../development/commands.md).
...
| Generované přehledy | Znovuvytvoření z verzovaného zdroje | Při každém buildu | Samostatná záloha není nutná | Není relevantní | 2026-08-28 `pnpm run docs:generate` a kontrola prošly |
...
Při diagnostice ověř po `pnpm install --frozen-lockfile` a čistém sestavení dostupnost `public/kitchen-pdf.mjs`, `public/pdfmake.min.js` a `public/vfs_fonts.js`.
...
| Vadná navigace nebo generátor | Revertuj poslední příčinný commit nebo dodej kompatibilní opravu | Zdrojové recepty nemaž kvůli odvozené chybě | `pnpm test`, build a průchod katalogem | Engineering při nejasné migraci cest |
